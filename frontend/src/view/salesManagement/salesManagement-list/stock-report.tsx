import {SalesManagement} from "@data/salesManagement";
...
export type SalesManagementType = (typeof SalesManagement)[number];
...
export default function StockReport({ className }: { className?: string }) {
  const { table, setData } = useTanStackTable<SalesManagementType>({
    ...
    options: {
      ...
      meta: {
        handleDeleteRow: (row) => {
          setData((prev) => prev.filter((r) => r.productId !== row.productId));
        },
      },
